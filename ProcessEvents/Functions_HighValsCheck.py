    ...
    for _, combo_row in results_df.iterrows():
        ...
        for event_num in event_nums_to_check:
            ...
            try:
                ...
                max_bc, peak_idx_bc, ts_bc = get_event_series(
                    rainfall_cube_dir_bc, bc=True
                )

                max_nonbc, peak_idx_nonbc, ts_nonbc = get_event_series(
                    rainfall_cube_dir_nonbc, bc=False
                )
                ...
                if plot:
                    timesteps = np.arange(len(ts_bc))

                    fig, ax_bc = plt.subplots(figsize=(10, 2))
                    ax_nonbc   = ax_bc.twinx()
                    ...
                    ax_nonbc.plot(
                        ...
                        color='darkorange',
                        linewidth=1.5,
                        label=f'Non-BC (max={max_nonbc:.1f}mm)'
                    )
                    ...
                    ax_bc.axvline(
                        ...
                        linestyle=':',
                        alpha=0.7,
                        label=f'Non-BC peak (t{peak_idx_nonbc})'
                    )

                    # ── Axis styling ────────────────────────────────────
                    ax_bc.set_ylabel('BC max rainfall (mm)', color='steelblue')
                    ax_nonbc.set_ylabel('Non-BC max rainfall (mm)', color='darkorange')

                    ax_bc.tick_params(axis='y', labelcolor='steelblue')
                    ax_nonbc.tick_params(axis='y', labelcolor='darkorange')

                    ax_bc.set_xlabel('Timestep')

                    # ── Combined legend ─────────────────────────────────
                    lines  = ax_bc.get_lines() + ax_nonbc.get_lines()
                    labels = [l.get_label() for l in lines]
                    ax_bc.legend(lines, labels, fontsize=8, loc='upper left')

                    # ── Title (restored) ────────────────────────────────
                    fig.suptitle(
                        f"Catchment {catchment_num} — EM{ens_num} — Year {year}, Event {event_num}\n"
                        f"BC max: {max_bc:.1f}mm | Non-BC max: {max_nonbc:.1f}mm | "
                        f"Peak agrees: {peak_idx_bc == peak_idx_nonbc}",
                        fontsize=10
                    )
                ...
                print(f"  Failed for event {event_num}: {e}")
                ...
                rows.append({
                    ...
                    'event_num'            : event_num,
                    'max_value_bc'         : None,
                    'max_value_nonbc'      : None,
                    'peak_timestep_bc'     : None,
                    'peak_timestep_nonbc'  : None,
                    'peak_timestep_agrees' : None,
                    'peak_timestep_offset' : None,
                    'bc_nonbc_diff'        : None,
                })
