...
def plot_peak_check(sm_cube, catchment_poly, this_event_results, timestep=1):
    """
    Sanity check plot:
    ...
    sm_x = sm_cube.coord('projection_x_coordinate').points
    sm_y = sm_cube.coord('projection_y_coordinate').points
    dx   = sm_x[1] - sm_x[0]
    dy   = sm_y[1] - sm_y[0]

    fig, ax = plt.subplots(figsize=(4, 4), subplot_kw={'projection': ccrs.OSGB()})

    # Full SM field
    iplt.pcolormesh(sm_cube[timestep, :, :], axes=ax, cmap='Blues')
    ...
    # Centre point
    ax.scatter(x_coord, y_coord, s=100, color='red', zorder=6,
